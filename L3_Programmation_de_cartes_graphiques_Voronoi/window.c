...
int circleNOM(int x0, int y0, int r) {
  int x, y, del, incH, incO, t = M_SQRT1_2 * r + 1, acc = 0;
  del = 3 - (r << 1);
  incH = 6;
  incO = 10 - (r << 2);
  for(x = 0, y = r; x <= t; x++, incH += 4, incO += 4) {
    if(IN_SCREEN(x0 + x, y0 + y) && !gl4dpGetPixel(x0 + x, y0 + y)){ 
		gl4dpPutPixel(x0 + x, y0 + y); 
		acc =1;
	}
    if(IN_SCREEN(x0 + x, y0 - y) && !gl4dpGetPixel(x0 + x, y0 - y)){ 
		gl4dpPutPixel(x0 + x, y0 - y); 
    ...
    if(IN_SCREEN(x0 + y, y0 + x) && !gl4dpGetPixel(x0 + y, y0 + x)){ 
		gl4dpPutPixel(x0 + y, y0 + x); 
		acc = 1;
	}
    if(IN_SCREEN(x0 + y, y0 - x) && !gl4dpGetPixel(x0 + y, y0 - x)){ 
		gl4dpPutPixel(x0 + y, y0 - x); 
